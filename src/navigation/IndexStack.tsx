...
const IndexStack = () => {
  ...
  useEffect(() => {
    ...
    const unapprovedSnipesQuery = query(
      postsRef,
      and(where('target_id', '==', user.uid), where('approved', '==', false)),
    );
  ...
  if (user) {
    if (unapprovedSnipes.length > 0) {
      return (
        <ApprovalScreen
          unapprovedSnipes={unapprovedSnipes}
          setUnapprovedSnipes={setUnapprovedSnipes}
        />
      );
    } else {
      return <AppStack />;
